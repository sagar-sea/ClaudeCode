...
class OllamaClaudeLauncher:
    ...
    def build_content(self, parent):
        ...
        # Utilities
        self.create_utilities(container)
    ...
    def create_utilities(self, parent):
        """Utility functions"""
        card = self.create_card(parent, "★", "Utilities")

        util_row = tk.Frame(card, bg=self.card_bg)
        util_row.pack(fill="x", pady=5)

        self.update_claude_btn = tk.Button(util_row, text="🔄 Check Claude Update",
                                          bg="#0ea5e9", fg="white",
                                          font=("Segoe UI", 10),
                                          relief="flat", cursor="hand2",
                                          command=self.check_claude_update,
                                          padx=15, pady=5)
        self.update_claude_btn.pack(side="left", padx=(0, 10))

        self.model_info_btn = tk.Button(util_row, text="ℹ️ Model Info",
                                       bg="#8b5cf6", fg="white",
                                       font=("Segoe UI", 10),
                                       relief="flat", cursor="hand2",
                                       command=self.show_model_info,
                                       padx=15, pady=5)
        self.model_info_btn.pack(side="left")
    ...
    def log(self, message, level="info"):
        ...
        if level == "error":
            color_tag = "error"
            self.log_text.tag_configure(color_tag, foreground="#f87171")
        elif level == "success":
            color_tag = "success"
            self.log_text.tag_configure(color_tag, foreground="#4ade80")
        elif level == "warning":
            color_tag = "warning"
            self.log_text.tag_configure(color_tag, foreground="#fbbf24")
        else:
            color_tag = "info"
            self.log_text.tag_configure(color_tag, foreground="#93c5fd")
    ...
    def refresh_models(self):
        ...
        if model_type == "local":
            try:
                ...
                self.log(f"Found {len(models)} local model(s)", "success")
            except Exception as e:
                self.log(f"Error refreshing models: {e}", "error")
            ...
            self.log("Loaded cloud models", "success")
            ...
            try:
                ...
                for i in range(30):
                    ...
                    if self.is_ollama_running():
                        ...
                        self.root.after(0, lambda: self.log("Server started!", "success"))
                        return

                self.root.after(0, lambda: self.log("Timeout waiting for server", "error"))
            except Exception as e:
    ...
    def run_model(self):
        ...
        def run():
            try:
                ...
                self.root.after(0, lambda: self.log(f"Model running: {model}", "success"))
            except Exception as e:
                self.root.after(0, lambda: self.log(f"Error running model: {e}", "error"))
    ...
    def launch_claude_code(self):
        ...
        try:
            ...
            self.log(f"Error launching Claude Code: {e}", "error")
    ...
    def check_claude_update(self):
        ...
        def check():
            try:
                self.root.after(0, lambda: self.log("Checking for Claude Code updates..."))
                result = subprocess.run([self.claude_path, "--version"],
                                      capture_output=True, text=True,
                                      creationflags=subprocess.CREATE_NO_WINDOW)
                current_version = result.stdout.strip()
                self.root.after(0, lambda: self.log(f"Current version: {current_version}", "success"))
                self.root.after(0, lambda: messagebox.showinfo("Version Info", f"Current Claude Code version:\n{current_version}"))
            except Exception as e:
                self.root.after(0, lambda: self.log(f"Error checking version: {e}", "error"))
    ...
    def show_model_info(self):
        ...
        def get_info():
            try:
                self.root.after(0, lambda: self.log(f"Getting info for model: {model}"))
                result = subprocess.run([self.ollama_cmd, "show", model],
                                      capture_output=True, text=True,
                                      creationflags=subprocess.CREATE_NO_WINDOW)
                if result.returncode != 0:
                    raise Exception(f"Ollama command failed: {result.stderr}")
                ...
                self.root.after(0, lambda: self.log(f"Model info retrieved", "success"))
                self.root.after(0, lambda: messagebox.showinfo("Model Info", f"Information for {model}:\n\n{info[:500]}..."))
            except Exception as e:
                self.root.after(0, lambda: self.log(f"Error getting model info: {e}", "error"))
